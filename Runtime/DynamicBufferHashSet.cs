...
namespace com.waywyrd.dynamicbuffer_hashset
{
    public struct DynamicBufferHashSet
    {
            ...
            where T : unmanaged, IBufferElementData, IEqualityComparer<T>
        {
            ...
            for (var i = Mathf.Max(capacity, MinBufferCapacity) - 1; i >= 0; i--)
            ...
            where T : unmanaged, IBufferElementData, IEqualityComparer<T>
        {
            ...
            var index = GetIndex(element, buffer.Capacity);

            if (hashMapBuffer[index].Empty)
            {
                var newElementIndex = buffer.Add(element);
            ...
            for (var i = 0; i < buffer.Capacity; i++)
            {
                if (nextIndex < 0)
                {
                    ...
                    hashMapBuffer[hashIndex] = new BufferHashSetElementData(hashMapBuffer[hashIndex].Index,
                        hashMapBuffer[hashIndex].Empty,
                        hashMapBuffer[hashIndex].PrevItem,
                        hashMapBuffer.Length - 1,
        ...
        public static bool TryRemove<T>(EntityManager entityManager, Entity entity, T element) where T : unmanaged, IBufferElementData, IEqualityComparer<T>
        {
            ...
            var itemIndex = FindBufferItemIndex(buffer, hashMapBuffer, element);
        ...
        private static void RemoveItemAt<T>(int index, DynamicBuffer<T> buffer, DynamicBuffer<BufferHashSetElementData> hashMapBuffer)
            where T : unmanaged, IBufferElementData, IEqualityComparer<T>
        {
            var lastItemIndex = buffer.Length - 1;
        ...
        private static void SetHashItemIndex(ref DynamicBuffer<BufferHashSetElementData> hashMapBuffer, int index, int bufferItemIndex)
        {
            ...
            var hashSetItem = hashMapBuffer[hashMapBufferIndex];

            for (var i = 0; i < hashMapBuffer.Capacity; i++)
            {
        ...
        {
            var hashSetItem = hashMapBuffer[bufferItemIndex];
        ...
        private static void RemoveHashItemInChain(ref DynamicBuffer<BufferHashSetElementData> hashMapBuffer, int hashMapIndex, int chainLastIndex)
        {
            ...
            else
            {
                ...
                {
                    var prevLastIndex = lastItem.PrevItem;
            ...
            where T : unmanaged, IBufferElementData, IEqualityComparer<T>
        {
            ...
            var itemBufferIndex = hashMapBuffer[index].Index;

            if (buffer[itemBufferIndex].Equals(element, buffer[itemBufferIndex]))
                return itemBufferIndex;

            var nextIndex = hashMapBuffer[index].NextItem;
            for (var i = 0; i < buffer.Capacity; i++)
            {
                if (nextIndex < 0)
                    return -1;
